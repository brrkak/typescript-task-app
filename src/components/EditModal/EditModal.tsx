...
import { deleteTask, setMdoalActive, updateTask } from '../../store/slices/boardSlice';
import { v4 } from 'uuid';
import { addLog } from '../../store/slices/loggerSlice';
import { buttons, closeButton, deleteButton, header, input, mdoalWindow, title, updateButton, wrapper } from './EditModal.css';
...
const EditModal = () => {
  ...
  const handleDelete = () =>{
    ...
    dispatch(
      addLog({
        logId:v4(),
        logMessage: `일 삭제하기: ${editingState.task.taskId}`,
        logAuthor: "User",
        logTimeStamp : String(Date.now())
      })
    )
